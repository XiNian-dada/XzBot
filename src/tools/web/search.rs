//! 内置搜索实现：负责 Bing / SearXNG 搜索、结果清洗与排序。
//!
//! 搜索模块承担的不是“简单抓网页”，而是尽量把搜索结果整理成适合模型消费的摘要。
//! 因此这里包含：
//! - 搜索会话预热与二段式查询
//! - SearXNG / Bing 等搜索源适配
//! - 结果过滤、排序、天气站点优先级、标识符严格匹配
//! - 搜索结果缓存与预览抓取

use super::*;

/// 执行网页搜索，并把结果整理成适合模型消费的紧凑摘要。
...
fn is_weather_related_hit(hit: &SearchHit) -> bool {
    ...
    let keywords = [
        "天气", "预报", "一周", "7天", "15天", "30天", "气温", "weather", "forecast", "tianqi",
    ];
